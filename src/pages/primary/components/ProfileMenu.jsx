...
export const ProfileMenu = ({ user }) => {
  ...
  const renderProfileButton = () => (
    <button
      type="button"
      className={`size-8 rounded-full border border-border bg-secondary text-primary font-bold hover:scale-110 hover:bg-hover md:size-10`}
      id="user-menu-button"
      aria-expanded="false"
  ...
  const renderProfileMenu = () => (
    <div
      ...
      } absolute -right-10 z-50 my-2 list-none divide-y divide-border rounded-lg bg-secondary text-base shadow md:right-0 dark:divide-border-dark dark:bg-secondary-dark`}
      id="user-dropdown"
    >
      <div className="px-4 py-3">
        <span className="block text-sm text-black ">{`${user?.firstName} ${user?.lastName}`}</span>
        <span className="block truncate text-sm text-black">{user?.email}</span>
      </div>
      <ul className="py-2" aria-labelledby="user-menu-button">
        <li>
          <button
            type="button"
            onClick={onLogout}
            className="block w-full px-4 py-2 text-sm text-primary hover:bg-hover dark:text-light-dark dark:hover:bg-hover-dark dark:hover:text-light-dark"
          >
            {common.signOut}
          </button>
        </li>
